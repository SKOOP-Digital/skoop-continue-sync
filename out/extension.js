...
function forceDocsRetryForAgents(agents) {
...
function generateAgentYaml(agent) {
    let yaml = '';
        ...
        for (const model of agent.models) {
            ...
            if (model.defaultCompletionOptions) {
                ...
                if (opts.temperature !== undefined)
                    yaml += `      temperature: ${opts.temperature}\n`;
                if (opts.maxTokens)
                    yaml += `      maxTokens: ${opts.maxTokens}\n`;
            }
        }
        yaml += '\n';
    }
    ...
    if (agent.context && agent.context.length > 0) {
        ...
        for (const contextItem of agent.context) {
            if (contextItem.uses) {
                yaml += `  - uses: ${contextItem.uses}\n`;
            }
            else if (contextItem.provider) {
                yaml += `  - provider: ${contextItem.provider}\n`;
            }
        }
        yaml += '\n';
    }
    ...
    if (agent.mcpServers && agent.mcpServers.length > 0) {
        ...
        for (const mcpServer of agent.mcpServers) {
            if (mcpServer.uses) {
                yaml += `  - uses: ${mcpServer.uses}\n`;
            }
            else {
                yaml += `  - name: "${mcpServer.name}"\n`;
                if (mcpServer.type)
                    yaml += `    type: ${mcpServer.type}\n`;
                if (mcpServer.url)
                    yaml += `    url: ${mcpServer.url}\n`;
                if (mcpServer.command)
                    yaml += `    command: ${mcpServer.command}\n`;
                if (mcpServer.args) {
                    yaml += '    args:\n';
                    for (const arg of mcpServer.args) {
                        yaml += `      - "${arg}"\n`;
                    }
                }
            }
...
function applyRulesAndPrompts(config, teamConfig) {
    ...
    for (const rule of teamRules) {
        ...
        if (rule.uses) {
            const existingRuleIndex = config.rules.findIndex((r) => r.uses === rule.uses);
    ...
    for (const prompt of teamPrompts) {
        // Handle hub blocks (uses syntax)
        if (prompt.uses) {
            const existingPromptIndex = config.prompts.findIndex((p) => p.uses === prompt.uses);
    ...
    if (config.models && config.models.length > 0) {
        ...
        for (const model of config.models) {
            if (model.uses) {
                ...
                if (model.override) {
                    yaml += '    override:\n';
                    if (model.override.roles) {
                        yaml += '      roles:\n';
                        for (const role of model.override.roles) {
                            yaml += `        - ${role}\n`;
                        }
                    }
                }
                ...
                if (model.defaultCompletionOptions) {
                    ...
                    if (opts.contextLength)
                        yaml += `      contextLength: ${opts.contextLength}\n`;
                    if (opts.maxTokens)
                        yaml += `      maxTokens: ${opts.maxTokens}\n`;
                }
            }
