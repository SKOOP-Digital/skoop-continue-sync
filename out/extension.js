...
async function enableAutoAccept() {
    ...
    try {
        ...
        // Reset the setting
        await vscode.workspace.getConfiguration('skoop-continue-sync').update('autoAcceptContinueDialogs', false, vscode.ConfigurationTarget.Global);
    }
}
/**
...
async function disableAutoAccept() {
    ...
    try {
        ...
        if (fs.existsSync(globalContextPath)) {
            ...
            if (globalThis.originalToolPolicies) {
                globalContext.toolPolicies = globalThis.originalToolPolicies;
                delete globalThis.originalToolPolicies;
            }
    ...
    vscode.window.showInformationMessage('Skoop Auto-Accept DISABLED. Reload window for changes to take effect.', 'Reload Window').then(selection => {
        if (selection === 'Reload Window') {
            vscode.commands.executeCommand('workbench.action.reloadWindow');
        }
    });
