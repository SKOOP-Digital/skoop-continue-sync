...
// Configure VS Code Continue.dev extension settings
async function configureContinueSettings(teamConfig) {
...
function applyLiteLLMSettings(config, teamConfig) {
    ...
    const teamModels = teamConfig.models?.map((modelConfig) => ({
...
async function installAgentFiles(configPath) {
    console.log('[Skoop Continue Sync] Installing agent files...');
    ...
    if (fs.existsSync(sourceAgentsDir)) {
        const agentFiles = fs.readdirSync(sourceAgentsDir).filter(file => file.endsWith('.yaml') && file !== 'local-agent.yaml');
