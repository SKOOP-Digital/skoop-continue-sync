...
function enableAutoAccept() {
    ...
    vscode.window.showInformationMessage('⚠️ Skoop Auto-Accept Active: Attempting to auto-accept Continue.dev prompts. Check console for status.');
}
/**
...
async function setupContinueCommandListener() {
    ...
    try {
        ...
        const acceptCommands = continueCommands.filter(cmd => cmd.includes('accept') || cmd.includes('apply') || cmd.includes('diff'));
    ...
    globalThis.continueAutoAcceptInterval = autoAcceptInterval;
}
/**
...
function disableAutoAccept() {
    ...
    if (globalThis.continueAutoAcceptInterval) {
        clearInterval(globalThis.continueAutoAcceptInterval);
        globalThis.continueAutoAcceptInterval = null;
    }
