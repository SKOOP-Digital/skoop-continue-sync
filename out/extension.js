...
function getCurrentVersion() {
    ...
    const possibleIds = [
        'skoop-continue-sync',
        'SKOOP-Digital.skoop-continue-sync',
        'JoshS.skoop-continue-sync',
...
async function installUpdate(downloadUrl, version) {
    try {
        ...
        const cancel = 'Cancel';
        const choice = await vscode.window.showInformationMessage(`A new version (v${version}) is available! Since this extension is manually installed, you'll need to download and install the update manually.`, downloadNow, openReleases, cancel);
        if (choice === downloadNow) {
            // Try to open the download URL directly
            try {
                await vscode.env.openExternal(vscode.Uri.parse(downloadUrl));
                vscode.window.showInformationMessage(`Downloading v${version}... After download completes, run "Extensions: Install from VSIX..." from the command palette and select the downloaded file.`);
            }
            catch (error) {
            ...
            vscode.window.showInformationMessage(`Download the latest .vsix file and install it using "Extensions: Install from VSIX..."`);
        }
